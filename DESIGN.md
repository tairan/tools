...
字体堆栈与桌面角色见前置 token。Tairan Serif 由本站 `/fonts/tairan-serif.woff` 加载，使用 `font-display: swap`，声明字重范围 400–600。工具标题采用系统无衬线，而目录引言采用宋体。没有统一等比字号阶梯。日文界面的正文使用本地字体 Hiragino Kaku Gothic ProN、Yu Gothic、Noto Sans CJK JP、sans-serif，品牌继续保留宋体。
...
页头提供品牌主页链接、博客链接、外观与全站语言选择（简体中文、English、日本語）。语言切换保留当前操作状态，控件按译文长度自然换行。手机隐藏页头博客链接，页尾仍有主站入口。分类筛选是带计数的普通文字链接，当前项以强调色文字与底线表示；它们不是胶囊标签。工作页面包屑和相关工具均为真实链接。
